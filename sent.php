...
// Get folder from query string or default to 'sent'
$folder = isset($_GET['folder']) ? $_GET['folder'] : 'sent';

// Define valid folders
$valid_folders = ['inbox', 'unread', 'draft', 'sent', 'archive', 'trash', 'spam', 'starred'];
if (!in_array($folder, $valid_folders)) {
    $folder = 'sent'; // Default to 'sent' if the folder is not valid
}

// Fetch emails for the logged-in user based on folder
$user_id = $_SESSION['user_id'];

// Prepare the SQL statement based on the folder
if ($folder === 'starred') {
    ...
    // Fetch emails for other folders (including 'sent')
    $stmt = $pdo->prepare('
        SELECT e.id, e.sender, e.recipient, e.subject, e.body, e.status, e.created_at, u.email AS sender_email
        FROM emails e
        JOIN users u ON e.user_id = u.id
...
<div class="side-panel">
    ...
    <div class="navigation">
        ...
        <a href="starred.php" class="<?php echo $folder === 'spam' ? 'active' : ''; ?>">
...
<div class="main-content">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr onclick="openViewEmailModal('<?php echo htmlspecialchars($email['recipient']); ?>', '<?php echo htmlspecialchars($email['subject']); ?>', '<?php echo htmlspecialchars($email['created_at']); ?>', '<?php echo addslashes(htmlspecialchars($email['body'])); ?>')">
                <td>
                    <a href="starred.php?email_id=<?php echo htmlspecialchars($email['id']); ?>">
...
<!-- The View Email Modal -->
<div id="view-email-modal" class="view-email-modal" style="display:none;">
    <div class="view-email-modal-content">
        <span class="view-email-modal-close" onclick="closeViewEmailModal()">&times;</span>
        <div class="view-email-modal-header">
            <h2>Email Details</h2>
        </div>
        <div class="view-email-modal-body">
            <strong>To:</strong> <span id="view-email-modal-recipient"></span><br>
            <strong>Subject:</strong> <span id="view-email-modal-subject"></span><br>
            <strong>Date:</strong> <span id="view-email-modal-date"></span><br>
            <strong>Body:</strong>
            <p id="view-email-modal-body"></p>
        </div>
    </div>
</div>

<script>
    function openViewEmailModal(recipient, subject, date, body) {
        document.getElementById('view-email-modal-recipient').innerText = recipient;
        document.getElementById('view-email-modal-subject').innerText = subject;
        document.getElementById('view-email-modal-date').innerText = date;
        document.getElementById('view-email-modal-body').innerHTML = body.replace(/\n/g, '<br>');
...
<style>
    ...
    .view-email-modal-close:hover {
        color: #f00; /* Change color on hover */
        cursor: pointer;
    }
    .view-email-modal-body {
